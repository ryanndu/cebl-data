...
def update_pbp_data():
    ...
    try:
        pbp = pd.read_csv(
            "https://github.com/ryanndu/cebl-data/releases/download/pbp/"
            + "cebl_pbp_"
            + str(current_year)
            + ".csv"
        )
        current_schedule = current_schedule[
            ~current_schedule["fiba_id"].isin(pbp["game_id"])
        ]
    ...
    for _, row in current_schedule.iterrows():
        ...
        try:
            ...
            new_pbp = pd.concat(
                [new_pbp, extract_pbp_data(json_data)], ignore_index=True
            )
        except Exception as e:
...
def update_officials_data():
    ...
    current_schedule = current_schedule.query("season == @current_year")
    current_schedule = current_schedule[
        ~current_schedule["fiba_id"].isin(officials["game_id"])
    ]
    ...
    for _, row in current_schedule.iterrows():
        ...
        try:
            ...
            json_data["game_id"] = game_id
            json_data["season"] = season
            new_officials = pd.concat(
                [new_officials, extract_officials_data(json_data)], ignore_index=True
            )
        except Exception as e:
            print(f"Error for game_id {game_id}: {e}")
            continue
...
def update_coaches_data():
    ...
    current_schedule = current_schedule.query("season == @current_year")
    current_schedule = current_schedule[
        ~current_schedule["fiba_id"].isin(coaches["game_id"])
    ]
    ...
    for _, row in current_schedule.iterrows():
        ...
        try:
            ...
            json_data["game_id"] = game_id
            json_data["season"] = season
            new_coaches = pd.concat(
                [new_coaches, extract_coach_data(json_data)], ignore_index=True
            )
        except Exception as e:
            print(f"Error for game_id {game_id}: {e}")
            continue
...
def update_players_data():
    ...
    current_schedule = current_schedule.query("season == @current_year")
    current_schedule = current_schedule[
        ~current_schedule["fiba_id"].isin(players["game_id"])
    ]
    ...
    for _, row in current_schedule.iterrows():
        ...
        try:
            ...
            new_players = pd.concat(
                [new_players, extract_player_data(json_data)], ignore_index=True
            )
        except Exception as e:
            print(f"Error for game_id {game_id}: {e}")
            continue
...
def update_team_data():
    ...
    current_schedule = current_schedule.query("season == @current_year")
    current_schedule = current_schedule[
        ~current_schedule["fiba_id"].isin(teams["game_id"])
    ]
    ...
    for _, row in current_schedule.iterrows():
        ...
        try:
            ...
            json_data["season"] = season
            new_teams = pd.concat(
                [new_teams, extract_team_data(json_data)], ignore_index=True
            )
        except Exception as e:
            print(f"Error for game_id {game_id}: {e}")
            continue
